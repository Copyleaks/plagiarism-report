    ...
    app.service('statisticsService', ["settingsService", "utilitiesService", "logService", function (settingsService, utilitiesService,logService) {
        ...
        function getSourcesWithMatches(sources, checkIfActive) {
            ...
            for (var i = 0; i < sourcesLength && i < maxSourcesToCompute; i++) {
                ...
                if (source.hasOwnProperty('matchesBeforeSplit')) {
                    if (!checkIfActive)
                        result.push(source);
        ...
        this.atLeastOneSourceWithMatches = function (sources) {
            ...
            for (var sourceKey in sources) {
                ...
                if (source.hasOwnProperty('matchesBeforeSplit') && source.matchesBeforeSplit.Identical != null) {
                    return true;
                }
            }
        ...
        function getIdenticalRanges(sourcesWithMatches) {
            return underscore.flatten(
                underscore.map(sourcesWithMatches, function (source) {
                    return source.matchesBeforeSplit.Identical;
                }));
        }

        function getSimilarRanges(sourcesWithMatches) {
            return underscore.flatten(
                underscore.map(sourcesWithMatches, function (source) {
                    return source.matchesBeforeSplit.Similar;
                }));
        }
        ...
        function getRelatedRanges(sourcesWithMatches) {
            return underscore.flatten(
                underscore.map(sourcesWithMatches, function (source) {
                    return source.matchesBeforeSplit.RelatedMeaning;
                }));
        }
        ...
        function mergeSourceMatches(sources, wordCount) {
            ...
            if (settingsService.instance.showIdentical) {
                identiaclMatches = getIdenticalRanges(sourcesWithMatches);
            }

            var similarMatches = [];
            if (settingsService.instance.showSimilar) {
                similarMatches = getSimilarRanges(sourcesWithMatches);
            }

            var relatedMatches = [];
            if (settingsService.instance.showRelated) {
                relatedMatches = getRelatedRanges(sourcesWithMatches);
            }
        ...
        function mergeRanges(matchRanges,priority) {
            ...
            for (var i = 0; i < matchRanges.length; ++i) {
                stack.push({
                    wordIndex: matchRanges[i].SoWS,
                    type: 0
                });
                stack.push({
                    wordIndex: matchRanges[i].SoWE,
                    type: 1
                });
            }
