    ...
    app.service('pageService', ['settingsService', 'DataService','reportDataService','contentTypeService',
        function (settingsService, DataService, reportDataService, contentTypeService) {
            var service = {
                ...
                convert_comparison_report_with_pages: convert_comparison_report_with_pages,
            ...
            function highlightText(params) {
                ...
                if (!matches || matches.length == 0)
                    return [];
                matches.excluded = underscore.sortBy(matches.excluded, function (m) { return m.match.SoE });
                ...
                var text = _highlightText(text, ranges, sources, mapType, reportType, disableTooltip, contentType, pageType);
                return text;
            }
            ...
            function _highlightText(text, ranges, sources, mapType, reportType, disableTooltip, contentType, pageType) {
                ...
                for (var i = 0; i < ranges.length; ++i) {
                    ...
                    if (range.matchType == 4) {
                        ...
                        var textToAdd = text.substr(range.SoS, range.SoE - range.SoS);
                        var preText = "<span" +
                            getClassesForHighlightedElement(range.matchType, contentType, range.ids) +
                             getClickAction(contentType, pageType, range.ids) +
							tooltip +
                            ">";
            ...
            function getClickAction(contentType, pageType, rangeIds) {
                var rangeIdsString = getRangeIdsStringForClickParameter(rangeIds);

                if (contentTypeService.isHtmlContentType(contentType)) {
                    if (!pageType) pageType = 'test';
                    return " onclick=\"cpyc(" + rangeIdsString + ", '" + pageType + "');\"";
                }

                return " ng-click=\"clicked(" + rangeIdsString + ");\"";
            }
            ...
            function getClassesForHighlightedElement(matchType, contentType,rangeIds) {
            ...
            function mergeMatches(pageLength, matches, mapType) {
                ...
                for (var i = 0; i < matches.identical.length; ++i) {
                    match = matches.identical[i];
                    uniqueId = 'i_' + match.id + '_' + match.match.GID;
                    for (var x = match.match[fromStart]; x < match.match[fromEnd]; ++x) {
                        positions[x].push({
                            matchType: 1,
                ...
                for (var i = 0; i < matches.similar.length; ++i) {
                    match = matches.similar[i];
                    uniqueId = 's_' + match.id + '_' + match.match.GID;
                    for (var x = match.match[fromStart]; x < match.match[fromEnd]; ++x) {
                        positions[x].push({
                            matchType: 2,
                            id: uniqueId //matches.similar[i].id
                        });
                    }
                }

                for (var i = 0; i < matches.relatedMeaning.length; ++i) {
                    match = matches.relatedMeaning[i];
                    uniqueId = 'r_' + match.id + '_' + match.match.GID;
                    for (var x = match.match[fromStart]; x < match.match[fromEnd]; ++x) {
                        positions[x].push({
                            matchType: 3,
                            id: uniqueId//matches.relatedMeaning[i].id,
                        });
                    }
                }

                for (var i = 0; i < matches.excluded.length; ++i) {
                    match = matches.excluded[i].match;
                    for (var x = match[fromStart]; x < match[fromEnd]; ++x) {
                        positions[x].push({
                            matchType: 4,
            ...
            function updateCurrentPageMatches(sources, pageIndex, excludedRangesPerPage) {
                ...
                for (var item in sourcesToLookForMatches) {
                    ...
                    if (!source.is_active || source.status != DataService.eSourceStatus.completed
                        || !source.matches
                        || !source.matches.identical
                        || !source.matches.identical[pageIndex]
                        || !source.matches.relatedMeaning
                        || !source.matches.relatedMeaning[pageIndex]
                        || !source.matches.similar
                        || !source.matches.similar[pageIndex])
                        continue;

                    

                    if (settings.showIdentical) {
                        for (var i = 0; i < source.matches.identical[pageIndex].length; ++i) {
                            current_page_matches.identical.push(
                                {
                                    id: source.id,
                                    match: source.matches.identical[pageIndex][i]
                                }
                            );
                        }
                    }
                    ...
                    if (settings.showSimilar) {
                        for (var i = 0; i < source.matches.similar[pageIndex].length; ++i) {
                            current_page_matches.similar.push(
                                {
                                    id: source.id,
                                    match: source.matches.similar[pageIndex][i]
                                }
                            );
                        }
                    }
                    ...
                    if (settings.showRelated) {
                        for (var i = 0; i < source.matches.relatedMeaning[pageIndex].length; ++i) {
                            current_page_matches.relatedMeaning.push(
                                {
                                    id: source.id,
                                    match: source.matches.relatedMeaning[pageIndex][i]
                                }
                            );
                        }
                    }
                ...
                if (excludedRangesPerPage && excludedRangesPerPage.length > 0) {
                    for (var i = 0; i < excludedRangesPerPage[pageIndex].length; ++i) {
                        current_page_matches.excluded.push(
                            {
                                id: -1,
                                match: excludedRangesPerPage[pageIndex][i]
                            }
                        );
                    }
                }
            ...
            function convert_comparison_report_with_pages(source, pageRanges, mapType) {
                if (!pageRanges || pageRanges === null)
                    throw ('pageRanges cannot be null');

                if (source.comparisonVersion < 2 || !reportDataService.sourceHasGids(source)) {
                    reportDataService.fixOldTextScanGids(source);
                }

                if (!source.matchesBeforeSplit) source.matchesBeforeSplit = jQuery.extend(true, {}, source.matches);
                var spliter = new PagesSpliter();
                source.matches = {
                    identical: spliter.split(source.matches.Identical || source.matches.identical, pageRanges, mapType),
                    similar: spliter.split(source.matches.Similar || source.matches.similar, pageRanges, mapType),
                    relatedMeaning: spliter.split(source.matches.RelatedMeaning || source.matches.relatedMeaning, pageRanges, mapType),

                    identicalWordsCounter: source.matchesBeforeSplit.IdenticalCopiedWords,
                    similarWordsCounter: source.matchesBeforeSplit.SimilarCopiedWords,
                    relatedWordsCounter: source.matchesBeforeSplit.RelatedMeaningCopiedWords
                };
